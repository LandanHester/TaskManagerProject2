...
while (userInput !== `CLOSE`){
  ...
  if (userInput === `REMOVE`){
    ...
    // Verifies the user entered a whole number (AKA Not a decimal) that one of the options in the prompt (AKA Within the number range from 1 to the length of the tasks array)
    // The 1st condition (Math.floor(num) !== num) checks to see if the user entered a decimal
    // The 2nd condition (num < 0) checks to see if the user entered in 0 (Zero) or a negative number
    // The 3rd condition (num >= tasks.length) checks to see if the user entered in a number that is higher than those that were displayed to the user
    // The 4th condition (!num) checks to see if the user entered in something that cannot be converted into a number which causes the value of NaN (Not A Number) to be set to the num variable (NOTE: NaN has a default boolean value of false)
